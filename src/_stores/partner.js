      ...
      try {
        await http.post(`${baseUrl}/list`, params).then(resp => {
          ...
          if (resp.data.code === 200) {
            this.partner_list = resp.data.body;
            this.partner_list_total = resp.data.total;
            //console.log("get member_list", resp.data)

            // 페이징 셋팅
            this.endPage = Math.ceil(this.partner_list_total / this.page_block)
            this.pagesList = [];
